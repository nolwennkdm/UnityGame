...
public class PlayerSword : MonoBehaviour
{
    ...
    private bool hasHitMonster = false; // Flag pour vérifier si le monstre a déjà pris des dégâts pendant l'attaque
    ...
    void AttachSword()
    {
        ...
        {
            // Instancier l'épée et la positionner sur la main du personnage
            equippedSword = Instantiate(swordPrefab, swordAttachmentPoint.position, swordAttachmentPoint.rotation, swordAttachmentPoint);

            // Assurez-vous que l'épée reste à l'échelle correcte et positionnée sur la main
            ...
            // Si vous avez besoin d'un Rigidbody ou Collider pour l'épée (optionnel)
            Rigidbody swordRb = equippedSword.GetComponent<Rigidbody>();
            if (swordRb == null)
            {
                swordRb = equippedSword.AddComponent<Rigidbody>();
                swordRb.isKinematic = true; // Éviter les collisions et la physique avec le joueur
            }

            // Vous pouvez aussi ajouter un Collider si nécessaire
            Collider swordCollider = equippedSword.GetComponent<Collider>();
            if (swordCollider == null)
            {
                swordCollider = equippedSword.AddComponent<BoxCollider>();
                swordCollider.isTrigger = true; // Assurez-vous que le collider est un trigger pour la détection de collision
            }
        }
        else
        {
